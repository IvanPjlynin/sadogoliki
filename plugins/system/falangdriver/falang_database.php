...
class JFalangDatabase extends JOverrideDatabase {
	...
	function getNumRows( $cur=null, $translate=true, $language=null )
	{
		...
		if ($defaultLang == $language){
			$rows = array($count);
            Factory::getApplication()->triggerEvent('onBeforeTranslation', array (&$rows, &$ids, $reference_table, $language, $refTablePrimaryKey, $this->getRefTables(), $this->sql, $allowfallback));
			$count = $rows[0];
			return $count;
		}
        ...
        Factory::getApplication()->triggerEvent('onAfterTranslation', array (&$rows, &$ids, $reference_table, $language, $refTablePrimaryKey, $this->getRefTables(), $this->sql, $allowfallback));
		$count = $rows[0];
		return $count;
	}
