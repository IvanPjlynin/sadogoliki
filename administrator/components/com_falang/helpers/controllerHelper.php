...
*/
     ...
     * @since version 4.0.5
     */
    static function _checkPluginFields(){
        $db     = Factory::getDbo();
        $query  = $db->getQuery(true);
        ...
        $query->where($query->quoteName('type') . '=' . $query->quote('plugin'));
        $query->where($query->quoteName('folder') . '=' . $query->quote('system'));
        $query->where($query->quoteName('element') . 'IN ("fields","falangdriver")');
        $query->order('ordering ASC');

        $db->setQuery($query);
        $list = $db->loadObjectList('element');

        if (isset($list['fields']) and isset($list['falangdriver'])){
            if ((int)$list['fields']->ordering >=  (int)$list['falangdriver']->ordering){
                //display a message
                $fields_plugin = PluginHelper::getPlugin('system', 'fields');
                $falangdriver_plugin = PluginHelper::getPlugin('system', 'falangdriver');

                //do nothing if fields plugin or falangdriver not enabled.
                if (empty($fields_plugin) || empty($falangdriver_plugin)  ){return true;}

                Factory::getApplication()->enqueueMessage(Text::_('COM_FALANG_PLUGINS_SYSTEM_FIELDS_FAILED'), 'notice');
            }
        }
    }
